...
class MyMainForm(QMainWindow, Ui_Form):
    ...
    signal_display_image_and_recognize = pyqtSignal(str, socket.socket)  # 图片路径和当前客户端
    ...
    def __init__(self, parent=None):
        ...
        # self.close_Button.clicked.connect(self.tcp_close)  # 關閉服務

        self.tcp_signal_write_info.connect(self.receive_display)
        self.tcp_signal_ip_and_port.connect(self.tcpServer_start)
        self.signal_display_image_and_recognize.connect(self.display_image_and_recognize)
    ...
    #
    #             self.label_client_num.setText(str(len(self.client_socket_list)))
    #
    #         # 轮询客户端套接字列表，接收数据
    ...
    #                     image_path = 'test' + time_now + '.jpg'
    #                     f = open(image_path, "ab")  # filepath为你要存储的图片的全路径
    #                     f.write(recv_msg)  # 将图片数据写入文件，保存到文件夹
    #                     f.close()
    #
    #                     # 接收數據，判斷接收完成，需要和stm32 WiFi發送端發送數據時進行處理
    #                     # 保存完再提交信号，进行显示和识别
    #                     ######## 这里的判断条件需要考虑，要根据和client端约定的传输条件进行判断，是根据文件字节大小还是约定结束标志
    #                     if b'\xff\xd9' in recv_msg:
    #                         # 等接收完數據後再提交消息，通過判斷是否收到ff d9判斷是否接收完
    #                         self.signal_display_image_and_recognize.emit(image_path, client)  # 發送消息，進行顯示和識別
    #
    #                     # print(type(client))
    #                     # 保存完图片，发送消息
    #                     # 传一个信号给执行人脸识别的函数，信号的参数：把当前client传过去，这样识别完以后就返回结果给传来图片的client
    #                     # client.send("ok".encode("utf-8"))
    #
    #                 else:
    #                     client.close()
    #                     self.client_socket_list.remove((client, address))   # 当client断开连接，将该client的信息移除列表
    #                     print(address[0] + "：" + str(address[1]) + " 已断开连接")
    #                     # print(len(self.client_socket_list))
    #
    #                     self.label_client_num.setText(str(len(self.client_socket_list)))
    ...
    def tcp_server_concurrency(self):  # 数据接收，只接收str
        ...
        while True:
            ...
            for client, address in self.client_socket_list:
                try:
                    # recv_msg = client.recv(4096)     # 读取4096个字节的数据
                    recv_msg = client.recv(4096)   # 先接收4字节

                except Exception as ret:
                    pass
                else:  # try如果没有异常，则执行else这一块
                    if recv_msg:
                        info = int.from_bytes(recv_msg, "big",signed=False)
                        print(info)
                        print(type(info))
                        # info = recv_msg.decode("utf-8")
                        # print("来自" + address[0] + ":" + str(address[1]) + "的消息:")
                        ...
                        msg = f"来自IP:{address[0]}端口:{address[1]}:\n"
                        self.tcp_signal_write_info.emit(msg, str(info))
                        # self.tcp_signal_write_info.emit(msg)
                        # self.tcp_signal_write_info.emit(info)
                    else:
                        client.close()
    ...
    def display_image_and_recognize(self, path: str, client: socket.socket):
        self.label_image.setPixmap(QPixmap(path))  # 顯示接收到的圖片

        # 顯示後做識別並給出結果

        client.send("ok".encode("utf-8"))  # 返回结果
